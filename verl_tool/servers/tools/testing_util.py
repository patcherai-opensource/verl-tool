...
def run_test(in_outs, test=None, debug=False, timeout=15):
    ...
    if test is None:
        ...
        for index, inputs in enumerate(in_outs["inputs"]):
            # inputs: represented as a single string
            # outputs: represented as a list of strings
            inputs = str(inputs)
            in_outs["outputs"] = [str(x) for x in in_outs["outputs"]]  # ensure outputs are strings
