    ...
    def conduct_action(self, trajectory_id, action, extra_field):
        ...
        if not is_valid:
            ...
            if self.force_run_test_cases and test_cases is not None:
                ...
                res, error_msg = check_correctness(in_outs=test_cases, generation=parsed_action)
                
                # print(f"res: {res}, error_msg: {error_msg}")

                # type: <listproxy> containing a dict
                # handle uncaught global_timeout error
                error_msg = error_msg[0] if isinstance(error_msg, list) and len(error_msg) > 0 else {}
...
def check_correctness(in_outs: Optional[dict], generation, timeout=5, debug=True):
